...
<template>
  <!-- ── SESSION PICKER ─────────────────────────────── -->
  <div v-if="view === 'select'" class="wood-bg">
    <div class="picker-screen">
      <div class="top">
        <img class="hero" src="/tu-hero.png" alt="Classes" />
        <div class="title">Tu & Valu</div>
      </div>
...
.student-circle.is-open {
  box-shadow:
    0 0 0 3px rgba(255, 200, 60, 0.8),
    0 4px 20px rgba(60, 28, 4, 0.5);
}

@keyframes flash-tap {
  0% {
    background: rgba(255, 220, 80, 0.95);
    box-shadow: 0 0 24px rgba(255, 200, 50, 0.9);
  }
  100% {
    background: rgba(255, 248, 220, 0.88);
  }
...
.student-circle.flash {
  animation: flash-tap 0.4s ease-out forwards;
}
...
.skill-bubble:hover {
  background: rgba(232, 168, 32, 0.85);
  color: #1a0e04;
  border-color: #e8a820;
}
.skill-bubble:active {
